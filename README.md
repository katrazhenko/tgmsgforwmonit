...
```
Telegram-групи → [Адмін-чат?] → [Minus-слова] → [Keyword-фільтр] → [AI-фільтр GPT] → Черга → Канал-дестинація
```

1. Бот слухає **всі** вхідні повідомлення в групах, де присутній акаунт
2. **Виключає чати адмінів** зі списку моніторингу
3. Відкидає повідомлення, що містять **мінус-слова** (`minus_words`)
4. Перевіряє наявність **ключових слів** (`keywords`)
5. За бажанням — пропускає через **GPT**, який вирішує: цільове чи спам
6. Цільові повідомлення потрапляють у **чергу** та пересилаються у вказаний канал із затримкою
...
- `python-dotenv` (для збереження ключів)
...
| `admins` | Username (з `@`) з правом на команди. Чати з адмінами виключені з моніторингу |
| `ai_filter_enabled` | Увімкнути GPT-фільтрацію |
| `ai_main_filter_role` | Роль/системний промпт для AI-фільтрації |
| `ai_tagret_filter_criteria` | Критерії цільового повідомлення |
| `ai_spam_filter_criteria` | Критерії спаму |
...
| `/ai_set_role <текст>` | Задати роль/системний промпт AI |
| `/ai_get_role` | Переглянути поточну роль |
| `/ai_set_target <текст>` | Критерії цільового повідомлення |
| `/ai_get_target` | Переглянути критерії цільового |
| `/ai_set_spam <текст>` | Критерії спаму |
| `/ai_get_spam` | Переглянути критерії спаму |
...
├── test_main.py                     # Юніт-тести (45 тестів)
...
**45 тестів, 0 помилок.** Покривають: `clean_minus_words`, `has_minus_word`, `find_keyword`, `format_sender`, `format_chat`, `is_admin`, edge-cases (unicode, punctuation, accents), інтеграційні сценарії.
